...
class ExtendPath2D:
    ...
    def CalculateConnectionStep2(self, gridFrom: OneGrid, gridTo: OneGrid) -> [bool, complex]:
        ...
        for i in range(0, len(consideringX) - 1):
            [bHasRes, _] = self.IntegrateCube(consideringX[i], consideringX[i + 1], gridFrom, gridTo)
            if not bHasRes:
                return [False, 0j]
        return [True, 0j]
